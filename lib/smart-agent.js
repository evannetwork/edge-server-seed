...
module.exports = class SmartAgent {
   ...
   * ensureSearchAuth) Usual `ensureEvanAuth` middleware has no runtime context and cannot ensure
   * identity authorization.
   */
  registerAuthMiddleware(authName = (this.config.name || 'identity'), runtime = this.runtime) {
    const camelCaseAuthName = `${authName[0].toUpperCase()}${authName.slice(1, authName.length)}`

    api.actions.addMiddleware({
      name: `ensure${camelCaseAuthName}Auth`,
      global: false,
      priority: 10,
   ...
   */
  async _getHeaders () {
    const signingAccount = this.runtime.underlyingAccount || this.config.ethAccount
